...
public class ProductIdentityTests
{
    ...
    public void AppServerHandshakeUsesCurrentProductIdentity()
    {
        ...
        Assert.Equal("CycleArc.Core", typeof(CodexProtocol).Assembly.GetName().Name);
    }
    ...
    public void LocalizedProductLabelsUseCurrentName(UiLanguage language)
    {
        ...
        try
        {
            ...
            var tooltip = CycleArcPresentation.Tooltip(CodexQuotaSnapshot.Empty(CodexQuotaStatus.SignedOut));
    ...
    public void RenamePreservesExistingStorageAndInstallationKeys()
    {
        ...
        Assert.Equal("CodexMeter", LegacyInstallation.PreviousStartupValueName);
        Assert.Equal("CodexMeter.exe", LegacyInstallation.PreviousExecutableFileName);
