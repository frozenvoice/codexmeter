...
public class CodexOnlyTests
{
    ...
    public void UnavailableIdentityDoesNotExposeSavedPercentage(CodexQuotaStatus status)
    {
        ...
        Assert.DoesNotContain("25", CycleArcPresentation.CompactText(snapshot));
        Assert.DoesNotContain("25%", CycleArcPresentation.Tooltip(snapshot));
    }
    ...
    public void CachedUsageIsExplicitlyMarkedStale()
    {
        ...
        Assert.Contains("25%", CycleArcPresentation.CompactText(snapshot));
        Assert.Contains("~", CycleArcPresentation.CompactText(snapshot));
        Assert.Contains(CycleArcPresentation.StatusLabel(snapshot), CycleArcPresentation.Tooltip(snapshot));
    }
