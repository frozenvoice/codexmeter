...
public class DeterministicExtensionIdTests
{
    ...
    public void SameManifestKey_ProducesSameId_RegardlessOfFilesystemPath()
    {
        ...
        var dirA = Path.Combine(Path.GetTempPath(), "cyclearc-tests", Guid.NewGuid().ToString("N"), "extension");
        var dirB = Path.Combine(Path.GetTempPath(), "cyclearc-tests", Guid.NewGuid().ToString("N"), "publish", "local", "extension");
    ...
    {
        var missing = Path.Combine(Path.GetTempPath(), "cyclearc-tests", Guid.NewGuid().ToString("N"), "manifest.json");
