...
public class CompanionHostManifestTests
{
    ...
    {
        var result = CompanionHostManifest.TryCreate(@"C:\missing\cyclearc.exe", "abcdefghijklmnopabcdefghijklmnop", null);
    ...
    {
        var host = Path.Combine(Path.GetTempPath(), "cyclearc-companion-host.exe");
    ...
    public void ValidManifest_ContainsOnlyApprovedOrigins()
    {
        ...
        try
        {
            ...
            Assert.DoesNotContain("cyclearc.exe", Path.GetFileName(result.HostPath), StringComparison.OrdinalIgnoreCase);
        }
        finally
        {
            File.Delete(host);
...
public class CompanionCallerOriginTests
{
    ...
    {
        string[] args = ["cyclearc-companion-host.exe", "chrome-extension://" + ChromeId + "/", "--parent-window=1234"];
        Assert.True(NativeMessagingHost.ShouldRun(args, Registered));
        Assert.True(CompanionCallerOrigin.IsAllowed(args, Registered));
    }
    ...
    {
        Assert.False(CompanionCallerOrigin.IsAllowed(
            ["cyclearc-companion-host.exe", "chrome-extension://NOT-VALID/"],
            Registered));
    }
    ...
    {
        Assert.False(CompanionCallerOrigin.IsAllowed([]));
        Assert.False(CompanionCallerOrigin.IsAllowed(["cyclearc-companion-host.exe"]));
        Assert.False(CompanionCallerOrigin.IsAllowed(["cyclearc-companion-host.exe", "--parent-window=9"], Registered));
    }
    ...
    {
        Assert.False(CompanionCallerOrigin.IsAllowed(
            ["cyclearc-companion-host.exe", "https://chatgpt.com/"],
            Registered));
    }
}
