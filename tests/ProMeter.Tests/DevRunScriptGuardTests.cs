...
public class DevRunScriptGuardTests
{
    ...
    public void Script_PublishesToStagingBeforeTouchingLocal()
    {
        ...
        Assert.True(stopProcessIndex >= 0, "dev-run.ps1 does not stop the running ProMeter process.");
    ...
    public void NoLaunch_ExitsBeforeStoppingOrDeployingAnything()
    {
        ...
        var stopProcessIndex = text.IndexOf("Stop-Process", StringComparison.Ordinal);
        var moveToLocalIndex = text.IndexOf("Move-Item -Path $StagingDir -Destination $LocalDir", StringComparison.Ordinal);
    ...
    private static string FindRepoRoot()
    {
        ...
        return dir?.FullName ?? throw new DirectoryNotFoundException("Could not locate repo root (ProMeter.sln) from " + AppContext.BaseDirectory);
    }
}
