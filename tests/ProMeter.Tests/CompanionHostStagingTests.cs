...
public class CompanionHostStagingTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "prometer-staging-" + Guid.NewGuid().ToString("N"));
    ...
    public void AdjacentRuntimeFiles_AreCopiedWithoutUiBinaries()
    {
        ...
        File.WriteAllText(Path.Combine(fakeRelease, "prometer-companion-host.dll"), "host-dll");
        File.WriteAllText(Path.Combine(fakeRelease, "prometer-companion-host.deps.json"), "{}");
        File.WriteAllText(Path.Combine(fakeRelease, "prometer-companion-host.runtimeconfig.json"), "{}");
        File.WriteAllText(Path.Combine(fakeRelease, "ProMeter.Core.dll"), "core");
        File.WriteAllText(Path.Combine(fakeRelease, "prometer.exe"), "ui");
        File.WriteAllText(Path.Combine(fakeRelease, "prometer-companion-host.pdb"), "symbols");
        ...
        Assert.True(File.Exists(Path.Combine(stagedDir, "prometer-companion-host.dll")));
        Assert.True(File.Exists(Path.Combine(stagedDir, "ProMeter.Core.dll")));
        Assert.False(File.Exists(Path.Combine(stagedDir, "prometer.exe")));
        Assert.False(File.Exists(Path.Combine(stagedDir, "prometer-companion-host.pdb")));
    ...
    {
        var source = File.ReadAllText(Find("src/ProMeter.Core/Companion/CompanionHostStager.cs"));
