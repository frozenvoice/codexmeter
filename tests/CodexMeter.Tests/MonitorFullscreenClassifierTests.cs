...
/// <summary>
/// Regression coverage for the multi-monitor fullscreen suppression bug: CodexMeter used to
...
public class MonitorFullscreenClassifierTests
{
    ...
    // D: fullscreen target-monitor window present; CodexMeter's own flyout/overlay becomes
    ...
    public void D_OwnOverlayBecomesForeground_StripStaysHidden()
    {
        var candidates = new WindowCandidateFacts[]
        {
            WindowCandidateFacts.Application(TargetMonitor4K, maximized: false, framed: false, popup: true, className: "GameWindowClass"),
            new WindowCandidateFacts(
                ClassName: "HwndWrapper[codexmeter.exe;;guid]",
    ...
    {
        var source = File.ReadAllText(Find("src/CodexMeter/UI/TaskbarWin32.cs"));
