...
public class CompanionHostStagingTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "codexmeter-staging-" + Guid.NewGuid().ToString("N"));
    ...
    public void AdjacentRuntimeFiles_AreCopiedWithoutUiBinaries()
    {
        ...
        File.WriteAllText(Path.Combine(fakeRelease, "codexmeter-companion-host.dll"), "host-dll");
        File.WriteAllText(Path.Combine(fakeRelease, "codexmeter-companion-host.deps.json"), "{}");
        File.WriteAllText(Path.Combine(fakeRelease, "codexmeter-companion-host.runtimeconfig.json"), "{}");
        File.WriteAllText(Path.Combine(fakeRelease, "CodexMeter.Core.dll"), "core");
        File.WriteAllText(Path.Combine(fakeRelease, "codexmeter.exe"), "ui");
        File.WriteAllText(Path.Combine(fakeRelease, "codexmeter-companion-host.pdb"), "symbols");
        ...
        Assert.True(File.Exists(Path.Combine(stagedDir, "codexmeter-companion-host.dll")));
        Assert.True(File.Exists(Path.Combine(stagedDir, "CodexMeter.Core.dll")));
        Assert.False(File.Exists(Path.Combine(stagedDir, "codexmeter.exe")));
        Assert.False(File.Exists(Path.Combine(stagedDir, "codexmeter-companion-host.pdb")));
    ...
    {
        var source = File.ReadAllText(Find("src/CodexMeter.Core/Companion/CompanionHostStager.cs"));
