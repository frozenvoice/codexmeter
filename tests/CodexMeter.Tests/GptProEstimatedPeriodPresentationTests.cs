...
public class GptProEstimatedPeriodPresentationTests
{
    ...
    {
        var source = File.ReadAllText(Find("src/CodexMeter/UI/FlyoutWindow.xaml.cs"));
    ...
    {
        var flyout = File.ReadAllText(Find("src/CodexMeter/UI/FlyoutWindow.xaml.cs"));
        Assert.DoesNotContain("ProStatusPresentation", flyout, StringComparison.Ordinal);
        var project = System.Xml.Linq.XDocument.Load(Find("src/CodexMeter/CodexMeter.csproj"));
    ...
    private static string Find(string relative)
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "CodexMeter.sln")))
        {
            dir = dir.Parent;
        }
