...
internal static class ClaudeStatusLineProcessChecks
{
    ...
    {
        executable ??= Path.Combine(RepositoryRoot(), "src", "CodexMeter", "bin", "Release",
    ...
    private static string RepositoryRoot()
    {
        for (var directory = new DirectoryInfo(AppContext.BaseDirectory); directory is not null; directory = directory.Parent)
            if (File.Exists(Path.Combine(directory.FullName, "CodexMeter.sln"))) return directory.FullName;
