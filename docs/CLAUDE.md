...
- A valid callback stays **Received** while idle, provided no included reset timestamp has passed and no receipt/cache/identity failure has occurred. There is no fixed age cutoff or attention warning for inactivity. The original receipt date/time remains visible. It records local delivery, not an authoritative source timestamp; the last-sample explanation does not promise a current account query.
...
- At an elapsed reset, or after clock rollback makes a sample future-dated, the last values remain visible as stale. The detail warning identifies the elapsed reset or invalid receipt time. CycleArc never rolls percentages back to zero locally. These checks and the original receipt survive application restarts.
