...
Profiles waiting for their first usage sample appear only in **Manage accounts**. The main account list, counts, attention indicator, tray and widget include the profile after usable quota data arrives. An explicit disconnection hides it again across restarts while preserving the last good cache and configuration location for reconnecting; a temporary delay instead keeps received values visible as stale.
