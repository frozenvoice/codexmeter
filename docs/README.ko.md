...
정상 입력이 **5분 동안 없거나 리셋 시각이 지나면** 마지막 정상값을 **오래된 데이터(stale)**로 표시합니다. 누락·잘못된 입력도 정상값을 지우지 않고 stale로 전환합니다. 상세 화면·계정 카드·위젯의 문구를 황색으로 강조하고 상세 링의 색도 바꿉니다. 재시작 후에도 마지막 수치를 보존합니다. 이 화면들과 트레이 툴팁에 **마지막 수신 날짜와 시각**을 표시하며, 긴 별명 때문에 트레이의 stale 상태·수신 시각·Web·Desktop·Code 공유 한도 안내가 잘리지 않게 합니다. 방금 받은 값도 **수신됨**으로 표시합니다. 이 시각은 로컬 수신 시각이며 계정의 실시간 조회 시각을 뜻하지 않습니다. 새로고침이나 로컬 조회로 수신 시각을 갱신하거나 Claude에 새 조회를 요청하지 않습니다. Claude에는 Codex 리셋권 기능을 표시하지 않습니다.

<details>
<summary><strong>첫 사용량 수신 후 Claude 화면 · 다크와 라이트</strong></summary>

<p>아래 예시에서는 실험용 Claude의 사용량을 받은 뒤 계정 3개가 표시됩니다. 선택된 실험용의 5시간 사용률은 91%, 주간 사용률은 47%이며 리셋 시각도 따로 표시합니다. 마지막 수신 후 12분이 지나 황색의 오래된 데이터 문구와 링으로 표시하며, 수신 날짜·시각과 마지막 수치를 유지합니다. 나머지 Codex 계정 2개는 최신 상태입니다.</p>
<table>
  <tr>
