...
3. Claude Code를 사용해 응답을 받으면 사용량이 들어옵니다. CycleArc에서 별도로 로그인한 계정은 **Claude Code 열기…**에서 작업 폴더를 선택해 실행하세요. 해당 계정의 설정으로 시작합니다. 공식 `rate_limits.five_hour` / `seven_day`의 `used_percentage`와 `resets_at`만 사용하며, 없는 구간은 0으로 표시하지 않습니다.
...
<p>아래 예시에서는 실험용 Claude의 사용량을 받은 뒤 계정 3개가 표시됩니다. 선택된 실험용의 5시간 사용률은 91%, 주간 사용률은 47%이며 리셋 시각도 따로 표시합니다. 마지막 수신 후 12분이 지나 이전 데이터로 표시하지만 값을 숨기지 않습니다. 나머지 Codex 계정 2개는 최신 상태입니다.</p>
<table>
  <tr>
    <td><img src="images/claude-overview-ko-dark.png" alt="Codex 2개와 선택한 실험용 Claude가 함께 보이며 5시간·주간 사용률과 오래된 수신 상태를 표시하는 다크 화면" width="440"></td>
...
**연결 상세 설정 → 연결 해제**는 기존 상태 표시줄을 복원하고 메인 화면에서 해당 프로필을 숨깁니다. 재시작해도 숨김 상태가 유지됩니다. 계정 관리의 프로필, 마지막 정상 캐시와 설정 위치는 남기며 Claude 자체를 로그아웃하지 않습니다. 다시 연결하면 새 공식 사용량을 받은 뒤 메인에 표시됩니다.
...
**메인에는 사용량을 받은 계정만 표시합니다.** 미연결 계정과 첫 사용량 대기 프로필은 **계정 관리**에 남고, 메인 계정 수나 확인 필요 개수에는 포함하지 않습니다. 첫 정상 데이터가 오면 자동으로 나타납니다. 사용량을 받은 뒤 일시적으로 업데이트가 늦어지는 계정은 이전 데이터로 계속 표시합니다. 표시할 계정이 하나도 없으면 팝업에 연결 안내를 보여주고 위젯은 숨깁니다.
...
<table>
  ...
  <tr>
    <td><img src="images/accounts-overview-ko-dark.png" alt="개인용·업무용 Codex 2개와 전체 최신 상태를 보여주고 사용량 대기 Claude는 숨긴 다크 화면" width="440"></td>
...
3. **대기 프로필은 계정 관리에 남습니다.** 실험용 Claude는 첫 사용량을 기다리므로 이 메인 화면에는 나오지 않습니다. 등록된 프로필은 3개여도 **계정 · 2**, **전체 최신**으로 표시합니다. 한 번 받은 값의 업데이트가 늦는 경우는 위 Claude 예시처럼 이전 데이터를 계속 보여줍니다.
...
<p>계정 관리에는 첫 사용량을 기다리는 실험용 Claude까지 프로필 3개가 남습니다. 실험용의 요약 카드는 아직 선택할 수 없지만 연결·별명 저장·순서 변경은 가능합니다. 메인 팝업은 사용량이 있는 Codex 2개만 셉니다. 순서를 바꿔도 선택한 업무용 계정은 유지됩니다.</p>
<p><img src="images/accounts-manage-ko-dark.png" alt="Codex 계정 2개와 데이터 대기 중인 실험용 Claude를 유지하며 연결 버튼을 제공하는 한국어 다크 계정 관리 화면" width="700"></p>
...
<p><img src="images/accounts-manage-ko-light.png" alt="같은 가상 프로필 3개와 선택한 업무용·대기 Claude의 연결 버튼을 보여주는 한국어 라이트 계정 관리 화면" width="700"></p>
