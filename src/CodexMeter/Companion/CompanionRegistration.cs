...
namespace CodexMeter.Companion;
...
public static class CompanionRegistration
{
        ...
        string? builtInExtensionId = null)
    {
        ...
        {
            return new CompanionHostManifestResult { Error = "codexmeter-companion-host.exe is missing" };
        }
    ...
    public static string WhaleInstructions =>
        "Whale is a Chromium browser, but CodexMeter does not invent a registry location for it. Use Chrome or Edge official Native Messaging registration, or follow Whale's own extension/native-host documentation.";
    ...
    public static string? ResolveHostPath(string trayExePath)
    {
        ...
        var companion = Path.Combine(directory, "codexmeter-companion-host.exe");
