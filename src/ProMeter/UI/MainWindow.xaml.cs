...
public partial class MainWindow : Window
{
        ...
        CodexQuotaSnapshot? codex = null)
    {
        ...
        PeriodText.Text = snapshot.SolProDailyLimit is int sol && snapshot.CombinedDailyLimit is int combined
            ? $"{UiText.HistoryStatistics} {DisplayFormatting.FormatDay(snapshot.PeriodStart)} – {DisplayFormatting.FormatDay(snapshot.PeriodEnd)}   ·   {UiText.ConfirmedProRequests} {presentation.ReconstructedText}   ·   {UiText.Gpt6Pro} {DisplayFormatting.WindowUsage(snapshot.UsesServerWeeklyCount ? snapshot.Used : snapshot.Gpt6WeeklyUsed, snapshot.Limit, snapshot.UsesServerWeeklyCount)}   ·   {UiText.SolPro} {DisplayFormatting.WindowUsage(snapshot.TodaySolPro, snapshot.SolProDailyLimit, snapshot.UsesServerSolDailyCount)}   ·   {UiText.CombinedDaily} {DisplayFormatting.WindowUsage(snapshot.CombinedToday, snapshot.CombinedDailyLimit, snapshot.UsesServerCombinedDailyCount)}"
