...
The last valid sample stays **Received** while idle, with its original **Last received** date and time. There is no five-minute warning for inactivity. **Stale data** appears only when its reported reset time passes, input is missing/malformed, or receipt/cache/identity checks fail. Those warnings use amber text on the detail card, account cards and widget, and an amber detail ring; the detail explains an elapsed reset or invalid receipt time. Percentages and receipt timestamps remain visible through restarts and in the tray tooltip. The shared Web·Desktop·Code scope stays visible because a received sample does not establish current server usage. Manual refresh and passive inbox reads do not renew receipt time, query Claude, or synthesize a new quota period. Claude has no Codex reset-credit controls.
