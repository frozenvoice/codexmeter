...
The last valid sample becomes **stale after five minutes without valid statusLine input**, when its reported reset time passes, or after missing/malformed input. **Stale data** appears in emphasized amber text on the detail card, account cards and widget; the detail ring also turns amber. The last percentages stay visible through restarts. **Last received** includes the local receipt date and time on these surfaces and in the tray tooltip, which keeps stale status and the Web·Desktop·Code shared scope ahead of a long nickname. Even recent input is labeled **Received**, since receipt time does not establish current server usage. Manual refresh and the passive inbox poll do not renew that time, query Claude, or synthesize a new quota period. Claude has no Codex reset-credit controls.

<details>
<summary><strong>Claude usage after the first sample · Dark and Light</strong></summary>

<p>Research now has received usage, so the same account list includes three accounts. Research is selected: its five-hour usage is 91% and seven-day usage is 47%, with separate reset times. The last sample is 12 minutes old and is visibly stale; its saved values remain on screen. The two Codex accounts still show current data.</p>
<table>
  <tr>
    <td><img src="docs/images/claude-overview-en-dark.png" alt="Dark mixed-provider popup with Research Claude selected, separate five-hour and weekly limits, a stale receipt and no Codex reset-credit card" width="440"></td>
