...
Only the official statusLine input is used for usage. Authentication uses the official `claude auth login --claudeai` and `claude auth status --json` commands. CycleArc does not parse `/usage`, read Claude auth/token files, launch a model turn, inspect transcripts or call an undocumented usage endpoint. It saves projected quota fields, receipt/status metadata and local connection paths/fingerprint, never the full stdin JSON. **Connection details → Disconnect** restores the previous status line. See [integration details](docs/CLAUDE.md) and the [official statusLine documentation](https://code.claude.com/docs/en/statusline).
